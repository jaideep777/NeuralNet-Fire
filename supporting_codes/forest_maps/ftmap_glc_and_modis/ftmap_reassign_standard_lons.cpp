...
/* --------------------------------------------------
compile command:
g++ -I/usr/local/netcdf-c-4.3.2/include -I/usr/local/netcdf-cxx-legacy/include -L/home/jaideep/codes/libgsm/lib -L/usr/local/netcdf-cxx-legacy/lib -o ftmap ftmap_reassign_standard_lons.cpp -lgsm -lnetcdf_c++  
-----------------------------------------------------*/
...
//	fti.printValues();	
	...
	gVar fto;
	fto.shallowCopy(fti);
	fto.setCoords(fti.times, levs, lats, lons);
	...
	for (int ilon=0; ilon<fti.nlons; ++ilon){
		for (int ilat=0; ilat<fti.nlats; ++ilat){
			...
			for (int ilev=0; ilev<fti.nlevs; ++ilev){
				levCounts[pftMap[ilev]] += fti.getCellValue(xlon,xlat,ilev);	// pftMap[ilev+1] for GLC, pftMap[ilev] for MOD  
			}	
			...
			cout << "(" << fto.lons[ilon] << ", " <<  fto.lats[ilat] << "): " << sum << endl;
	...
	fto_handle.open("ftmap_MOD12q1_0.5_9pft_sl.nc","w", glimits_globe);

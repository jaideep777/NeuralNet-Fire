...
int main(){
	...
	map<int, int> ftmap;
	for (int i=0; i<=13; ++i) ftmap[i]=i; // 0-13 are PFTs including water
	ftmap[16] = 14;		// 16 -> 14 is urban
	ftmap[255] = 0;		// 255 cells are from sea, so classify as water
	ftmap[254] = 15;		// put no data into 15 
...
//	string startdate = year + "-1-1", enddate = year + "-12-31";
	...
	vector <float> levs = createCoord(0, 15, 1, nlevs);
...
//	for (int i=0; i<ntimes; ++i) times[i] = gt0 - gtbase + i;
	...
	ifstream fin; fin.open("/media/jaideep/totoro/Work/Data/Fire-Data/MOD12Q1/mod12q1.txt");	

	NcFile_handle ft_handle;
	ft_handle.open("ftmap_MOD12q1_0.5.nc","w", glimits_globe);
...
//	vector <float> frac(23);
	int val;
	//40320 16353
	for (int iy=0; iy<21600; ++iy){
		for (int ix=0; ix<43200; ++ix){
//			getline(fin, s, ',');
//			int val = str2int(s);
			fin >> val;
			
			float lonc = -180 + ix*0.00833333333333333333;
			float latc = 90 -iy*0.00833333333333333;
			
			int ilon = indexC(ft.lons, lonc);
			int ilat = indexC(ft.lats, latc);
		
			ft(ilon, ilat, ftmap[val]) += 1;

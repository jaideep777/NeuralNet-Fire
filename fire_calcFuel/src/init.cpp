...
//	INIT_IP_MODELVAR( ffev,1); //   "ffev", ip_data_map["ffev"].unit, 1, mgtimes, log_fout); 
	...
	for (int i=0; i<model_variables.size(); ++i){
		...
		if (model_variables[i]->lwrite){	
			log_fout << vname << ", ";
			string fname = out_dir+"/"+vname+"."+sim_date0+"-"+sim_datef+".nc";
	...
	if (spout_on){
		log_fout << "> Opening file to write point values: " << (out_dir+"/"+pointOutFile) << '\n';
		point_fout.open((out_dir+"/"+pointOutFile).c_str());

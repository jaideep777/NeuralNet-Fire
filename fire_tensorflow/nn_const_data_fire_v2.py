...
X_ids = [ID_rh, ID_ts,  ID_wsp,  ID_dxl ,  ID_lmois]
n_inputs = len(X_ids)
...
my_data = genfromtxt('train_forest.csv', delimiter=',',skip_header=1)
print("DONE")
...
eval_data = genfromtxt('eval_forest.csv', delimiter=',',skip_header=1)
print(eval_data[0:5, X_ids+[ID_ba]])

print("Reading test data...")
test_data = genfromtxt('test_forest.csv', delimiter=',',skip_header=1)
...
xtest = tf.matmul(tf.cast(eval_data[:,X_ids], tf.float32), Wi) - bi
...
y_soft = tf.nn.softmax(denseNet(xeval, W1,b1,Wo,bo))
...
#        print(sess.run([accuracy, cross_entropy], feed_dict={it_handle: validation_handle}))
  ...
  y_val = sess.run(tf.nn.softmax(denseNet(tf.reshape(xin, [-1,n_inputs]),W1,b1,Wo,bo)))
  np.savetxt("y_predic_ba.txt",y_val,delimiter=" ")
